...
class WN(torch.nn.Module):
    ...
    def __init__(self, n_in_channels, n_mel_channels, n_layers, n_channels,
        ...
        cond_layer = torch.nn.Conv1d(n_mel_channels, 2 * n_channels * n_layers,
                                     1)
...
class WaveGlow(torch.nn.Module):
    ...
    def __init__(self, n_mel_channels, n_flows, n_group, n_early_every,
        ...
        for k in range(n_flows):
            ...
            self.WN.append(
                WN(n_half, n_mel_channels * n_group, n_layers_wn,
                   n_channels_wn, kernel_size_wn))
        self.n_remaining_channels = n_remaining_channels  # Useful during inference

    def forward(self, audio, spect):
        """
        spect = mel_spectrogram:  batch x n_mel_channels x frames
        audio = audio: batch x time
        """
    ...
    def infer(self, spect, sigma=1.0):
        ...
        for k in reversed(range(self.n_flows)):
            ...
            if k % self.n_early_every == 0 and k > 0:
                if spect.type() == 'torch.cuda.HalfTensor':
                    z = torch.cuda.HalfTensor(spect.size(0), self.n_early_size,
                                              spect.size(2)).normal_()
                else:
